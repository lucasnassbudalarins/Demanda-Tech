...
public class DemandaConsoleView implements CommandLineRunner {
        ...
        do {
            ...
            try { switch (op) {
                case 1 -> { 
                    ...
                    System.out.print("Descricao: "); 
                    String d = scanner.nextLine();
                    System.out.print("Matricula gerente (0=sem): "); 
                    Long gId = lerLong();
                ...
                case 2 -> {
                    departamentoService.buscarTudo().forEach(d -> System.out.println("ID=" + d.getIdDepartamento()
                    + " | " + d.getDescricao() + " | Ger=" + (d.getGerente() != null ? d.getGerente().getNome() : "N/A")));
                }
                case 3 -> { 
                    System.out.print("ID: "); 
            ...
            try { switch (op) {
                case 1 -> { 
                    ...
                    Funcionario f = funcionarioService.criar(
                        new FuncionarioCriarDTO(nome, email, dep), adm
                    );
                    System.out.println("Criado: Matricula=" + f.getMatricula()); }
                case 2 -> 
                    funcionarioService.buscarTudo().forEach(f -> System.out.println("Mat=" + f.getMatricula()
                        + " | " + f.getNome() + " | " + f.getEmail()
                        + " | Dep=" + (f.getDepartamento() != null ? f.getDepartamento().getDescricao() : "N/A")
                        + " | Admin=" + f.getAdmin())
                    );
                case 3 -> { 
                    System.out.print("Matricula: "); 
                    Funcionario f = funcionarioService.buscarPorId(lerLong());
                    System.out.println("Mat=" + f.getMatricula() + " | " + f.getNome() + " | " + f.getEmail()); }
                case 4 -> { 
                    Funcionario adm = loginAdmin(); 
                    System.out.print("Matricula: "); 
                    Long id = lerLong();
                    ...
                    System.out.print("Email: "); 
                    String email = scanner.nextLine();
                    System.out.print("ID Dep: "); 
                    Departamento dep = departamentoService.buscarPorId(lerLong());
                    funcionarioService.atualizar(id, new FuncionarioEditarDTO(nome, email, dep), adm);
                    System.out.println("Atualizado!"); }
                case 5 -> { 
                    Funcionario adm = loginAdmin(); 
                    System.out.print("Matricula: "); 
            ...
            try { switch (op) {
                ...
                case 2 -> {
                    tipoDemandaService.buscarTudo().forEach(
                        ...
                        + " | " + t.getDescricao() 
                        + " | Dep=" + (t.getDepartamento() != null ? t.getDepartamento().getDescricao() : "N/A")));
                }
                case 3 -> {
                    System.out.print("ID: ");
        ...
        do {
            ...
            try { switch (op) {
                case 1 -> {
                    ...
                    System.out.print("Matriculas envolvidos (virgula, ou vazio): "); String envStr = scanner.nextLine();
                    ...
                    System.out.println("Demanda criada: ID=" + dem.getIdDemanda() + " Resp=" + (dem.getResponsavel() != null ? dem.getResponsavel().getNome() : "auto")); }
                case 2 -> {
                    demandaService.buscarTudo().forEach(
                        d -> System.out.println("ID=" + d.getIdDemanda()
                        + " | " + d.getTitulo() + " | " + d.getData()
                        + " | St=" + (d.getStatus() != null ? d.getStatus().getDescricao() : "N/A")
                        + " | Pri=" + (d.getPrioridade() != null ? d.getPrioridade().getDescricao() : "N/A")
                        + " | Resp=" + (d.getResponsavel() != null ? d.getResponsavel().getNome() : "N/A")));
                    }
                case 3 -> { 
                    System.out.print("ID: "); 
                    Demanda d = demandaService.buscarPorId(lerLong());
                    System.out.println("ID=" + d.getIdDemanda() + " | " + d.getTitulo() + " | " + d.getDescricao()
                ...
                case 4 -> { 
                    ...
                    demandaService.atualizar(
                        id, new DemandaEditarDTO(titulo, null, null, desc, pri, tipo, criador)
                    );
                    System.out.println("Atualizado!"); 
                }
                case 5 -> { 
    ...
    private Funcionario loginAdmin() {
        ...
        return funcionarioService.buscarPorId(lerLong());
    }
